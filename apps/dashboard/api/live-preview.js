...
async function proxyPreviewAsset(asset, response) {
  ...
  if (normalizedType.includes("text/html") && /\.(?:css|m?js)$/.test(pathname)) {
    response.setHeader("Access-Control-Allow-Origin", "*");
    response.setHeader("Cross-Origin-Resource-Policy", "cross-origin");
    response.setHeader("Cache-Control", "private, no-store, max-age=0");
    return response.status(502).json({
      error: `The connected website returned HTML instead of the requested asset: ${url.pathname}. Republish the website assets and refresh the preview.`
    });
  }
  let body = bytes;
  if (
