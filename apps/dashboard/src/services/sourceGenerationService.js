...
function renderTree(tree){var content=document.getElementById('rcms-content');content.replaceChildren();(tree&&tree.children||[]).forEach(function(node){var item=render(node);if(item)content.appendChild(item)})}
